...
async function connect():Promise<Database>{let db:Database;
 ...
 else{if(process.env.NODE_ENV==='production')throw Error('DATABASE_URL PostgreSQL wajib untuk production.');const configuredDataDir=process.env.DATA_DIR?.trim();if(configuredDataDir)await mkdir(dataDir(),{recursive:true});const lite=configuredDataDir?new PGlite(path.join(dataDir(),'postgres')):new PGlite();await lite.waitReady;const wrap=(connection:any):Database=>({q:async(s,p=[])=>{const result=await connection.query(s,p);return result.rows;},tx:async(fn)=>connection.transaction((trx:any)=>fn(wrap(trx)))});db=wrap(lite);}
