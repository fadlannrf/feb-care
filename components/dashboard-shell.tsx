...
export function DashboardShell({children,user,demo}:{children:React.ReactNode;user:User;demo:boolean}){
 ...
 const menu=[{href:'/dashboard',label:'Ringkasan',icon:'grid'},...(!aggregate||user.role==='admin'||user.role==='leader'?[{href:'/dashboard/laporan',label:user.role==='student'?'Laporan saya':'Kelola laporan',icon:'message'}]:[]),...(user.role!=='student'?[{href:'/dashboard/analitik',label:'Analitik layanan',icon:'chart'}]:[]),{href:'/dashboard/notifikasi',label:'Notifikasi',icon:'bell'},{href:'/dashboard/pengaturan',label:user.role==='admin'?'Pengaturan':'Akun saya',icon:'settings'},...(user.role==='admin'?[{href:'/dashboard/audit',label:'Jejak audit',icon:'shield'}]:[])];
