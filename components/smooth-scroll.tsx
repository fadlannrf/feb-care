...
export function SmoothScroll(){const router=useRouter(),pathname=usePathname();useEffect(()=>{
 const isTicketDetail=pathname.startsWith('/dashboard/laporan/')&&pathname.length>'/dashboard/laporan/'.length;
 if(isTicketDetail){document.documentElement.classList.add('ticket-detail-native-scroll');return()=>document.documentElement.classList.remove('ticket-detail-native-scroll');}
